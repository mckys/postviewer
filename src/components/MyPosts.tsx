...
export const MyPosts = ({ onPostClick, refreshTrigger, updatedPostImageCount }: MyPostsProps) => {
  ...
  async function fetchUsername() {
    try {
      ...
      const username = userSettings?.civitai_username;
      ...
      if (username) {
        ...
        setLoading(false);
        setError('No username set. Please set your username in Settings.');
      }
    } catch (err) {
  ...
  async function fetchMyPosts(username: string) {
    try {
      setLoading(true);
      setError(null);
  ...
  async function toggleHide(postId: number, e: React.MouseEvent) {
    ...
    try {
      ...
      if (myUsername) fetchMyPosts(myUsername);
    }
  }
  ...
  async function handleSync() {
    try {
      setSyncing(true);
      console.log(`🔄 Syncing posts for @${myUsername}...`);

      // First, sync incomplete posts (posts with IDs but no images from extension)
      const { syncIncompletePosts } = await import('../lib/sync');
      console.log(`🔄 Syncing incomplete posts for ${myUsername}...`);
      await syncIncompletePosts(myUsername);
      ...
      await syncCreator(myUsername, undefined, { fullBackfill: true });

      // Refresh posts from database
      if (myUsername) await fetchMyPosts(myUsername);

      console.log(`✅ Sync complete for @${myUsername}`);
      alert(`Sync completed for ${myUsername}`);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-xl text-gray-600">Loading your posts...</div>
      </div>
    );
  }
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        <div className="text-xl text-gray-600">No posts found for @{myUsername}</div>
        <a
          href="#settings"
          className="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors"
        >
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-6 py-3 rounded-lg">
          ...
          {/* Divider */}
          <span className="text-3xl text-gray-900 -translate-y-0.5" style={{ fontWeight: 100 }}>|</span>
          ...
          <div className="flex items-center gap-4 h-9">
            ...
            <a
              href={`https://civitai.com/user/${myUsername}/posts`}
              target="_blank"
              rel="noopener noreferrer"
